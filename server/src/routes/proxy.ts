...
proxyRouter.post('/chat/completions', async (req: Request, res: Response) => {
  ...
  // Image requests run on vision-capable panel members (upstream added this;
  // the old `fusion_no_vision` guard is gone with it).
  //
  // Tool requests are rejected here: this is a LOCAL divergence from upstream
  // (which races tool-capable panel members and returns the first structured
  // tool call). Fusion is our commander-only path - it exists to fuse REASONING
  // turns, where a diverse panel plus a judge beats any single model. A tool
  // call is an action, not prose: it cannot be synthesized, so on a
  // tools-bearing request fusion silently collapses to a first-past-the-post
  // race with the judge skipped - all the token cost, none of the value, and
  // undetectable from the response. Workers do tool-calling via `auto` or a
  // pinned model; fusion never does. Failing loud keeps that topology invariant
  // enforced at the one chokepoint every client shares. See d6ab816.
